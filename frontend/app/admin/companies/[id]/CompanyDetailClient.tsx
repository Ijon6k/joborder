...
import { Company, CaseReview } from '@/lib/mockData';
...
export default function CompanyDetailClient({ company, cases }: ClientProps) {
  // Local state for mocking status updates
  const [currentStatus, setCurrentStatus] = useState(company.status);
  const [statusLog, setStatusLog] = useState(company.statusLog);
  const [selectedStatus, setSelectedStatus] = useState(company.status);
  const [inputNote, setInputNote] = useState('');
  const [showSuccess, setShowSuccess] = useState(false);

  // Calculate reviewed cases status
  const reviewedCases = cases.filter((c) => c.statusTinjauan === 'ditinjau');
  ...
  const submitStatusUpdate = () => {
    ...
    const newLog = {
      ...
      admin: 'Admin BP2MI',
      before: currentStatus === 'tidak_cukup_info' ? 'Tidak Cukup Info' : currentStatus,
      after: selectedStatus === 'tidak_cukup_info' ? 'Tidak Cukup Info' : selectedStatus,
      alasan: inputNote,
    };

    setCurrentStatus(selectedStatus);
    setStatusLog([newLog, ...statusLog]);
    setInputNote('');
    setShowSuccess(true);
  ...
  return (
    <div className="space-y-4 text-[#1d2327] font-normal">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-[#ccd0d4]">
        <h2 className="text-[20px] font-bold text-[#1d2327] leading-tight">
          {company.nama}
        </h2>
        <div>
          <span
          ...
          >
            {currentStatus === 'tidak_cukup_info' ? 'Tidak Cukup Info' : currentStatus}
          </span>
        </div>
      </div>
      ...
      {showSuccess && (
        <div className="bg-[#e8f7ee] border border-[#b6e3c6] text-[#0f7a3d] p-3 text-[13px] font-semibold rounded-none select-none">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
          ...
          <div className="bg-white border border-[#ccd0d4] p-5 rounded-none shadow-none space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-[13px]">
              <div>
                ...
                <span className="font-semibold text-zinc-800">{company.nib}</span>
              </div>
              <div>
                <span className="text-[11px] font-bold text-[#8a97a6] uppercase tracking-wider block mb-1">
                  Nomor SIP3MI
                </span>
                <span className="font-semibold text-zinc-800">{company.sip3mi}</span>
              </div>
            </div>
          </div>
                    ...
                    >
                      <div>
                        <Link
                          ...
                          className="font-bold text-[13.5px] text-[#1f5aa8] hover:text-[#163f78] block"
                        >
                          {cs.jenis}
                        </Link>
              ...
              <div className="space-y-3">
                {statusLog.map((log) => (
                  <div key={log.timestamp} className="p-3.5 border border-[#ccd0d4] bg-zinc-50/20 text-[13px] rounded-none">
                    <div className="flex items-center justify-between text-[11px] text-[#8a97a6] mb-1 font-semibold">
                      <span>{log.timestamp}</span>
                      <span>Diubah oleh: {log.admin}</span>
                    </div>
                    <div className="font-bold text-[#1d2327] mb-1 uppercase text-[12px]">
        ...
        <div className="lg:col-span-4 bg-white border border-[#ccd0d4] p-5 shadow-none rounded-none space-y-4">
          ...
          <div className="space-y-2">
            ...
            <div
              ...
              className={`p-3 border rounded-none flex flex-col relative select-none ${
                tciDisabled
                  ...
                  : selectedStatus === 'tidak_cukup_info'
                  ? 'border-[#54606e] bg-[#f8f9fa] cursor-pointer'
                  : 'border-zinc-200 hover:border-zinc-300 cursor-pointer'
              }`}
            >
              <div className="flex items-center justify-between">
                <span className="font-bold text-[13px] text-[#54606e]">Tidak Cukup Info</span>
                {tciDisabled ? (
                  <Lock className="w-3.5 h-3.5 text-zinc-400" />
                ) : (
                  selectedStatus === 'tidak_cukup_info' && <Check className="w-4 h-4 text-[#54606e]" />
                )}
              </div>
            ...
            <div
              ...
              className={`p-3 border rounded-none flex flex-col relative select-none ${
                netDisabled
                  ? 'opacity-40 border-zinc-200 bg-zinc-50 cursor-not-allowed'
                  : selectedStatus === 'netral'
                  ? 'border-green-600 bg-green-50/10 cursor-pointer'
                  : 'border-zinc-200 hover:border-zinc-300 cursor-pointer'
              }`}
            >
              <div className="flex items-center justify-between">
                <span className="font-bold text-[13px] text-green-700">Netral</span>
                {netDisabled ? (
                  <Lock className="w-3.5 h-3.5 text-zinc-400" />
                ) : (
                  selectedStatus === 'netral' && <Check className="w-4 h-4 text-green-700" />
                )}
              </div>
              <span className="text-[10px] text-zinc-400 mt-1 leading-normal">
            ...
            >
              <div className="flex items-center justify-between">
                <span className="font-bold text-[13px] text-red-700">Blacklist</span>
                {blDisabled ? (
                  <Lock className="w-3.5 h-3.5 text-zinc-400" />
                ) : (
                  selectedStatus === 'blacklist' && <Check className="w-4 h-4 text-red-700" />
                )}
              </div>
              <span className="text-[10px] text-zinc-400 mt-1 leading-normal">
          ...
          <button
            ...
            disabled={!inputNote.trim() || currentStatus === selectedStatus}
            className="w-full py-2.5 bg-[#1f5aa8] hover:bg-[#163f78] text-white text-[13px] font-bold transition-all duration-100 disabled:opacity-50 disabled:pointer-events-none rounded-none shadow-none flex items-center justify-center gap-1.5"
          >
